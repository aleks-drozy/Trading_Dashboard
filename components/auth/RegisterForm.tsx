...
export function RegisterForm() {
  ...
  const validateField = (field: "name" | "email" | "password", value: string) => {
    ...
    if (!result.success) {
      ...
      setFieldErrors((prev) => ({ ...prev, [field]: fieldError ? fieldError.message : undefined }))
    } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <div className="mb-1">
        <h1 className="text-2xl font-bold text-[#f8fafc] tracking-tight">Create account</h1>
        <p className="text-sm text-[#64748b] mt-1">Start tracking your trades today.</p>
      </div>
