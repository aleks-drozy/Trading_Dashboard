...
export function CandlestickChart({
  ...
  useEffect(() => {
    ...
    candleSeries.setData(bars.map((b) => ({ ...b, time: b.time as UTCTimestamp })))
    ...
    emaSeries.setData(ema.map((p) => ({ time: p.time as UTCTimestamp, value: p.value })))
    ...
    for (const zone of ifvgZones) {
      const zoneColor =
        zone.type === "bullish" ? "rgba(34, 197, 94, 0.5)" : "rgba(239, 68, 68, 0.5)"
      candleSeries.createPriceLine({
        price: zone.top,
        color: zoneColor,
        lineWidth: 1,
